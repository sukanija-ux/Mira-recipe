...
const AI_PROVIDERS = [
  {
    ...
    soft: 'oklch(0.96 0.025 35)',
    icon: '◆',
    description: 'Anthropic\'s Claude — thoughtful, nuanced, excellent at nutrition coaching.',
  },
  {
    id: 'openai',
    name: 'GPT-4o',
    maker: 'OpenAI',
    models: [{ id: 'gpt-4o-mini', label: 'GPT-4o mini (fast)' }, { id: 'gpt-4o', label: 'GPT-4o (best)' }],
    defaultModel: 'gpt-4o-mini',
    placeholder: 'sk-proj-…',
    keyPrefix: 'sk-',
    keyHint: 'platform.openai.com',
    color: 'oklch(0.56 0.09 160)',
    soft: 'oklch(0.95 0.025 160)',
    icon: '◉',
    description: 'OpenAI GPT-4o — widely used, strong general reasoning.',
  },
  {
    id: 'groq',
    name: 'Llama 3',
    maker: 'Groq',
    models: [{ id: 'llama-3.1-8b-instant', label: 'Llama 3.1 8B (fastest)' }, { id: 'llama-3.3-70b-versatile', label: 'Llama 3.3 70B (best)' }],
    defaultModel: 'llama-3.1-8b-instant',
    placeholder: 'gsk_…',
    keyPrefix: 'gsk_',
    keyHint: 'console.groq.com',
    color: 'oklch(0.52 0.12 260)',
    soft: 'oklch(0.95 0.025 260)',
    icon: '◈',
    description: 'Open-source Llama via Groq — lightning fast, free tier available.',
  },
];
...
function AIConnectBlock() {
  const [activeId,   setActiveId]   = useState_S(() => localStorage.getItem('mira_ai_provider') || 'claude');
  const [keys,       setKeys]       = useState_S(() => {
    try { return JSON.parse(localStorage.getItem('mira_ai_keys') || '{}'); } catch { return {}; }
  });
  ...
  const save = (id) => {
    const k = draft.trim();
    const newKeys = { ...keys, [id]: k };
    setKeys(newKeys);
    localStorage.setItem('mira_ai_keys', JSON.stringify(newKeys));
    localStorage.setItem('mira_ai_provider', id);
    setActiveId(id);
    setEditing(null);
    setDraft('');
  };
  ...
  const selectModel = (providerId, modelId) => {
    const m = { ...models, [providerId]: modelId };
    setModels(m);
    localStorage.setItem('mira_ai_models', JSON.stringify(m));
  };
  ...
  const disconnect = (id) => {
    ...
    localStorage.setItem('mira_ai_keys', JSON.stringify(newKeys));
    if (activeId === id) {
      const fallback = AI_PROVIDERS.find(p => p.id !== id && newKeys[p.id]);
      const next = fallback ? fallback.id : '';
      setActiveId(next);
      localStorage.setItem('mira_ai_provider', next);
    }
  };
  ...
  return (
    <div style={{ marginBottom: 48 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 24 }}>
        <h2 style={{ fontFamily: 'Instrument Serif, serif', fontSize: 28, fontWeight: 400, color: 'oklch(0.28 0.040 145)', margin: 0 }}>
          AI <em>model</em>
        </h2>
        <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: 'oklch(0.54 0.035 135)' }}>
          Powers the Health Coach and recipe suggestions
        </span>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {AI_PROVIDERS.map(p => {
          ...
          const isExpanded = editing === p.id;
          const chosenModel = models[p.id] || p.defaultModel;
          ...
          return (
            <div key={p.id} style={{
              ...
              border: isActive
                ? `1.5px solid ${p.color}`
                : isExpanded
                  ? '1.5px solid oklch(0.80 0.025 95)'
                  : '1px solid oklch(0.86 0.025 95)',
              background: isActive ? p.soft : 'oklch(0.97 0.015 90)',
              overflow: 'hidden',
              transition: 'all 0.2s ease',
            }}>
              {/* Card header row */}
              <div style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div style={{
                  ...
                  background: isActive ? p.color : 'oklch(0.90 0.020 90)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 18, color: isActive ? 'white' : 'oklch(0.54 0.035 135)',
                  transition: 'all 0.2s ease',
                }}>
                  {p.icon}
                </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 15, fontWeight: 600, color: 'oklch(0.28 0.040 145)' }}>
                      {p.name}
                    </span>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9.5, letterSpacing: '0.08em', color: 'oklch(0.56 0.030 135)' }}>
                      {p.maker}
                    </span>
                    {isActive && (
                      <span style={{
                        padding: '2px 9px', borderRadius: 999,
                        background: 'oklch(0.58 0.09 140)', color: 'white',
                    ...
                    {connected && !isActive && (
                      <span style={{
                        padding: '2px 9px', borderRadius: 999,
                        background: 'oklch(0.88 0.025 95)', color: 'oklch(0.50 0.030 135)',
                        fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.10em',
                      }}>CONNECTED</span>
                    )}
                  </div>
                  <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12.5, color: 'oklch(0.54 0.035 135)', marginTop: 3 }}>
                    {p.description}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                  {connected && !isActive && (
                    <button onClick={() => { setActiveId(p.id); localStorage.setItem('mira_ai_provider', p.id); }} style={{
                  ...
                  {connected && (
                    <button onClick={() => { setEditing(isExpanded ? null : p.id); setDraft(keys[p.id] || ''); }} style={{
                      padding: '7px 14px', borderRadius: 999, cursor: 'pointer',
                      background: 'transparent', color: 'oklch(0.50 0.030 135)',
                      border: '1px solid oklch(0.84 0.025 95)',
                      fontFamily: 'DM Sans, sans-serif', fontSize: 12.5,
                    }}>Edit key</button>
                  )}
                  {!connected && (
                    <button onClick={() => { setEditing(p.id); setDraft(''); }} style={{
                      padding: '7px 16px', borderRadius: 999, cursor: 'pointer',
                      background: 'oklch(0.28 0.040 145)', color: 'oklch(0.945 0.022 88)',
                      border: 'none',
                      fontFamily: 'DM Sans, sans-serif', fontSize: 12.5, fontWeight: 500,
                    }}>Connect</button>
                  )}
              ...
              {/* Expanded: key entry + model picker */}
              {isExpanded && (
                <div style={{ padding: '0 22px 22px', borderTop: '1px solid oklch(0.88 0.022 95)' }}>
                  <div style={{ paddingTop: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {/* Key input */}
                    <div>
                      <window.Eyebrow>API key</window.Eyebrow>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12.5, color: 'oklch(0.52 0.035 135)', margin: '4px 0 10px', lineHeight: 1.5 }}>
                        Get a free key at <strong>{p.keyHint}</strong> — it stays only in your browser.
                      </p>
                      <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        <button onClick={() => setEditing(null)} style={{
                          ...
                          fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: 'oklch(0.52 0.035 135)',
                        }}>Cancel</button>
                      </div>
                    </div>

                    {/* Model picker (shown after connecting) */}
                    {connected && (
                      <div>
                        <window.Eyebrow>Model</window.Eyebrow>
                        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                          {p.models.map(m => (
                            <button key={m.id} onClick={() => selectModel(p.id, m.id)} style={{
                              padding: '8px 14px', borderRadius: 999, cursor: 'pointer',
                              background: chosenModel === m.id ? p.color : 'transparent',
                              color: chosenModel === m.id ? 'white' : 'oklch(0.46 0.035 135)',
                              border: chosenModel === m.id ? `1px solid ${p.color}` : '1px solid oklch(0.84 0.025 95)',
                              fontFamily: 'DM Sans, sans-serif', fontSize: 12.5,
                            }}>{m.label}</button>
                          ))}
                        </div>
                    ...
                    {/* Disconnect */}
                    {connected && (
                      <button onClick={() => { disconnect(p.id); setEditing(null); }} style={{
                        alignSelf: 'flex-start', background: 'none', border: 'none', cursor: 'pointer',
                        fontFamily: 'DM Sans, sans-serif', fontSize: 12, color: 'oklch(0.60 0.09 25)',
                ...
                <div style={{ padding: '0 22px 14px', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  {p.models.map(m => (
                    <button key={m.id} onClick={() => selectModel(p.id, m.id)} style={{
                      ...
                      background: chosenModel === m.id ? p.color + '22' : 'transparent',
                      color: chosenModel === m.id ? p.color : 'oklch(0.58 0.030 135)',
                      border: chosenModel === m.id ? `1px solid ${p.color}55` : '1px solid oklch(0.88 0.022 95)',
      ...
      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 12, color: 'oklch(0.60 0.025 130)', marginTop: 14, lineHeight: 1.5 }}>
        Keys are stored only in your browser (localStorage) and never sent to Mira servers.
      </p>
    </div>
  );
}
